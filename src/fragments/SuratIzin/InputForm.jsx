...
import { faEnvelope, faPeopleLine } from "@fortawesome/free-solid-svg-icons";
...
export default function InputForm({ handleChange, surat, error }) {
  const template = {
    ...
    kode_pos: "59427",
    perihal: "Permohonan Izin Kuliah",
    kepada: "(Subjek)",
    // email: "E-Mail Aktif",
  ...
  return (
    <>
      ...
      <Accordion icon={faPeopleLine} label="Diberi izin">
        <TextArea label="Paragraf Perantara" placeholder="masukkan paragraf perantara" name="perantara" value={surat.perantara || template.perantara} handleChange={handleChange}></TextArea>
        <div className="my-2 grid grid-cols-2 grid-rows-1 w-full justify-around gap-2">
          <Input error={error.nama_mahasiswa} label="Nama Mahassiwa" placeholder="masukkan nama mahasiswa" name="nama_mahasiswa" value={surat.nama_mahasiswa || ""} handleChange={handleChange}></Input>
          <Input error={error.nim_mahasiswa} type="number" label="NIM Mahasiswa" placeholder="masukkan NIM mahasiswa" name="nim_mahasiswa" value={surat.nim_mahasiswa || ""} handleChange={handleChange}></Input>
        </div>
      </Accordion>
      ...
      <Accordion icon={faPenNib} label="Pengesahan">
        <div className="my-2 mb-10 grid grid-cols-2 grid-rows-3 w-full justify-around gap-2">
          ...
          <Input error={error.nim_ketua_organisasi} type="number" label="NIM Ketua Organisasi" placeholder="masukkan NIM ketua organisasi" name="nim_ketua_organisasi" value={surat.nim_ketua_organisasi || ""} handleChange={handleChange}></Input>
        </div>
      </Accordion>
    </>
  );
